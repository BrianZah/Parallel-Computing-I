...
// Preparing computation / communication
  ...
  MPI_Request req_send_1, req_recv_1;
  MPI_Request req_send_2, req_recv_2;
  MPI_Status s;
...
// Sending and receiving in x-direction
    ...
    if (sqrt_num-1 != x_pos)
    {
      ...
      MPI_Isend(Cols_send.data(), n_block*g, MPI_FLOAT, rank+1, i, MPI_COMM_WORLD, &req_send_1);
    }
    if ( 0 != x_pos)
    {
      MPI_Irecv(Cols_recv.data(), n_block*g, MPI_FLOAT, rank-1, i, MPI_COMM_WORLD, &req_recv_1);
      if (sqrt_num-1 != x_pos) MPI_Wait(&req_send_1, &s);
      extractBlock(Block.begin(), upper_margin, left_margin, n_block-left_margin+right_margin,
                  Cols_send.begin(), n_block, g);
      MPI_Isend(Cols_send.data(), n_block*g, MPI_FLOAT, rank-1, i, MPI_COMM_WORLD, &req_send_1);
      MPI_Wait(&req_recv_1, &s);
...
// Sending and receiving in y-direction
    if (sqrt_num-1 != y_pos)
    {
      ...
      MPI_Isend(Rows_send.data(), g*n_dim, MPI_FLOAT, rank+sqrt_num, i, MPI_COMM_WORLD, &req_send_2);
    }
    if ( 0 != y_pos)
    {
      MPI_Irecv(Rows_recv.data(), g*n_dim, MPI_FLOAT, rank-sqrt_num, i, MPI_COMM_WORLD, &req_recv_2);
      if (sqrt_num-1 != y_pos) MPI_Wait(&req_send_2, &s);
      extractBlock(Block.begin(), upper_margin, 0, 0,
                  Rows_send.begin(), g, n_dim);
      MPI_Isend(Rows_send.data(), g*n_dim, MPI_FLOAT, rank-sqrt_num, i, MPI_COMM_WORLD, &req_send_2);
      MPI_Wait(&req_recv_2, &s);
...
// Waiting for non-blocking send-operations
    MPI_Wait(&req_send_1, &s);
    MPI_Wait(&req_send_2, &s);
...
// Print (, sort, and save) results
  if (root == rank)
  {
    ...
    std::string filename = std::to_string(n) + "_" + std::to_string(g) + "_" + std::to_string(iter)
                         + "_non_blocking_bordered_time:_" + std::to_string(time_total) + ".ppm";
